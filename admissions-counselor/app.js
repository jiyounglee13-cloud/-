 ...
 * 데이터는 data.js(ADMISSIONS_DATA)에서 읽어오며, 이 파일은 수정 없이
 * 데이터만 갱신해도 동작합니다.
 * ===================================================================== */
...
const ENGLISH_ADVICE = {
  1: null,
  2: "영어 2등급: 상위권 대학은 1~2등급 차이가 감점 0.5~2점 수준입니다. 최상위권 지원 시 1등급 확보가 안전합니다.",
  3: "영어 3등급: 대학별 감점이 본격적으로 커집니다(3~5점 수준). 영어를 2등급 이상으로 올리는 것이 시급합니다.",
};

function englishAdvice(grade) {
  if (grade <= 1) return null;
  if (grade === 2) return ENGLISH_ADVICE[2];
  if (grade === 3) return ENGLISH_ADVICE[3];
...
// 대학별 종합 판정: 정시/교과 중 더 유리한 판정
function bestBand(uni, track, input) {
  const t = uni.tracks[track];
  if (!t) return null;
  const j = classifyJungsi(input.jungsiAvg, t.jungsi);
  const g = classifyNaesin(input.naesin, t.gyogwa);
  const candidates = [j, g].filter(Boolean).filter((b) => b.key !== "hard");
...
function buildUniversityList(input) {
  ...
  for (const uni of ADMISSIONS_DATA.universities) {
    const t = uni.tracks[input.track];
    if (!t) continue;
    const jungsi = classifyJungsi(input.jungsiAvg, t.jungsi);
    const gyogwa = classifyNaesin(input.naesin, t.gyogwa);
    const overall = bestBand(uni, input.track, input);
    rows.push({ uni, t, jungsi, gyogwa, overall });
  }
  rows.sort((a, b) => {
    const o = BAND_ORDER[a.overall.key] ?? 9;
    const p = BAND_ORDER[b.overall.key] ?? 9;
    if (o !== p) return o - p;
    return (b.t.jungsi ?? 0) - (a.t.jungsi ?? 0);
  });
  return rows;
}
...
function subjectCoaching(input, gap) {
  ...
  const needIfOne =
    weakest.name.startsWith("탐구") ? gap * 6 : gap * 3;
  tips.push(
...
function buildCoaching(uni, input) {
  ...
  if (!t) {
    result.verdict = `${uni.name}에는 ${input.track}계열 데이터가 없습니다.`;
    return result;
  }

  // ---- 정시 경로 ----
  if (t.jungsi != null) {
    const band = classifyJungsi(input.jungsiAvg, t.jungsi);
    const gap = t.jungsi - input.jungsiAvg;
    const route = {
      name: "정시 (수능 위주)",
      cutText: `국·수·탐 평균 백분위 약 ${t.jungsi} 필요 (추정)`,
    ...
    result.routes.push({
      name: "정시 (수능 위주)",
      cutText: "이 대학은 정시 일반전형 비중이 매우 낮거나 없습니다.",
      myText: "",
      band: null,
  ...
  // ---- 수시 교과 경로 ----
  if (t.gyogwa != null) {
    const band = classifyNaesin(input.naesin, t.gyogwa);
    const route = {
      name: "수시 학생부교과",
      cutText: `내신 약 ${t.gyogwa}등급 필요 (추정)`,
      myText: `내 내신 평균 ${input.naesin}등급`,
      band,
      tips: [],
    };
    const gap = input.naesin - t.gyogwa;
    if (gap <= 0) {
      route.tips.push(`내신이 합격선(추정)보다 ${Math.abs(gap).toFixed(2)}등급 좋습니다. 남은 학기 내신을 유지하고, 수능 최저학력기준 충족 여부를 반드시 확인하세요.`);
    } else {
      const r = requiredRemainingNaesin(input.naesin, input.completedSemesters, t.gyogwa);
      if (r.remaining === 0) {
        route.tips.push("내신 반영 학기(고3 1학기)가 모두 끝나 교과 성적을 더 올릴 수 없습니다. 학생부종합·논술·정시로 전략을 전환하세요.");
      } else if (!r.possible) {
        route.tips.push(
          `남은 ${r.remaining}개 학기에 전 과목 1.0등급을 받아도 목표 컷(${t.gyogwa})에 도달하기 어렵습니다. ` +
    ...
    result.routes.push({
      name: "수시 학생부교과",
      cutText: "이 대학은 학생부교과전형을 운영하지 않습니다.",
      myText: "",
      band: null,
      tips: [],
    });
  ...
  // ---- 수시 종합 경로 ----
  if (t.jonghap != null) {
    const looseBand = classifyNaesin(input.naesin, t.jonghap + 0.3); // 종합은 정성평가라 폭 넓게
    result.routes.push({
      name: "수시 학생부종합",
      cutText: `합격자 내신 평균 약 ${t.jonghap}등급대 (편차 큼)`,
      myText: `내 내신 평균 ${input.naesin}등급`,
      band: looseBand,
      tips: [
        "학생부종합은 내신 수치보다 <b>전공 연계 활동·세특(세부능력 및 특기사항)·탐구 경험</b>이 당락을 가릅니다.",
        "지망 학과와 연결되는 과목 선택(진로선택·융합선택 과목)과 수행평가·발표·독서 기록을 학기마다 누적하세요.",
        input.naesin > t.jonghap + 0.7
  ...
  if (scored.length) {
    ...
    const best = scored[0];
    result.verdict = `현재 성적 기준으로 <b>${uni.name} ${input.track}계열</b>은 <b>${best.name}</b> 경로가 가장 유리합니다 (판정: ${best.band.label}).`;
  } else {
    result.verdict = `현재 성적으로 <b>${uni.name} ${input.track}계열</b>은 모든 전형에서 합격선과 격차가 큽니다. 아래 로드맵대로 단계적으로 격차를 줄이거나, '적정' 판정 대학과 병행 지원 전략을 세우세요.`;
  }
...
function buildRoadmap(input) {
  ...
  items.push("모든 지원 전에 대입정보포털 '어디가(adiga.kr)'에서 해당 대학의 최신 입시결과와 모집요강을 반드시 확인하세요.");
  return items;
}
...
function renderList() {
  ...
  const summary = `
    <div class="summary">
      ...
      <div><span>계열</span><b>${input.track}</b></div>
    </div>`;
  ...
  const tr = rows.map((r) => `
    <tr>
      ...
      <td>${r.t.jungsi != null ? r.t.jungsi : "-"} ${bandChip(r.jungsi)}</td>
      <td>${r.t.gyogwa != null ? r.t.gyogwa + "등급" : "-"} ${bandChip(r.gyogwa)}</td>
    </tr>`).join("");
  ...
  box.innerHTML = `${summary}
    ...
    <p class="fine">판정 기준 — 안정: 여유 충분 / 적정: 합격선 부근 / 소신: 약간 부족 / 도전: 격차 큼 / 위험: 현재로선 어려움. 종합판정은 정시·교과 중 더 유리한 쪽입니다.</p>`;
  box.hidden = false;
